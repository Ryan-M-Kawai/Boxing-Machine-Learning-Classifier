...
FIRE_THRESHOLD  = 0.35   # tune live via the on-screen motion readout
...
with PoseLandmarker.create_from_options(options) as landmarker:
    while cap.isOpened():
        ...
        if latest_result and latest_result.pose_landmarks:
            ...
            # ── Punch model inference — swap model based on orientation,
            #    only run when a punch event is actually detected ──────
            if len(frame_buffer) == 30 and punch_armed and motion > FIRE_THRESHOLD:
                if current_sideways:
                    active_model   = model_sideways
                    active_le      = le_sideways
                    active_history = pred_history_sideways
                ...
                punch_armed = False  # lock out until arm returns near guard

            elif motion < RESET_THRESHOLD:
                punch_armed = True
            ...
            if displayed_label and time.time() < display_until:
                ...
                displayed_label = None
                draw_debug(frame, "no punch", 6, (0, 150, 150))
